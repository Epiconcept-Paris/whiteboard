...
function showProperties() {
  ...
  const aObjectValues = uObjectValues.merge(eObjectValues)
    .property("value", d => d).text(d => d).property("selected", function (d) {
      return d === d3.select(this.parentNode).datum().value;
    });
...
/**
 * Load action buttons properties and display for a visual
 */
...
function showAvailableProjects() {
  showModal("Select Epicraft Project", "Hello", [], () => {
  }, (div) => {
    storage.getWorkspaces((err, wks) => {
      drawWorkspaces(err, wks, div)
    });
  });
...
function drawWorkspaces(err, workspaces, div) {
  if (err == null) {
    ...
    update.enter()
      ...
      .on("click", (d) => {
        changeWorkspace(d);
        d3.select("div.modal").style("display", null);
      });
    update.exit().remove();
  }
}
...
function setupFields() {
  ...
  //existing groups
  groups.text(function (d) {
    return d.name;
  });
  //removed groups
  groups.exit().remove();

  var tables = groups.merge(newGroups).selectAll("div.table").data(function (d) {
    return d.tables
  });
  //New tables
  ...
  newTables.append("img")
    ...
    .attr("src", function (d) {
      if (d.collapsed) return "img/invertedTriangle.png"; else return "img/triangle.png";
    })
  ...
  //Existing Tables
  tables.text(function (d) {
    return d.name;
  });
  //Removed tables
  tables.exit().remove();

  var fields = tables.merge(newTables).selectAll("div.field").data(function (d) {
    return d.fields;
  })
  //New fields
  fields.enter()
    ...
    .call(fieldDraggable)
    .on("mouseover", function (d, id, group) {
      d3.select(group[id]).classed("focus", true)
    })
    .on("mouseout", function (d, id, group) {
      d3.select(group[id]).classed("focus", false)
    })
    .classed("fields-collapsed", function (d, id, group) {
      return group[id].parentNode.__data__.collapsed
    })
  ;
  //Existing fields
  fields.text(function (d) {
    return d.name;
  })
  //Removed fields
  fields.exit().remove();
...
function drawResizeLines(visuals) {
  existing = visuals.selectAll("div.resize").data(function (d) {
    return getResizers((d.x1 - d.x0), (d.y1 - d.y0))
  });
...
function getResizers(width, height) {
  ...
  return [
    ...
    , {
      ...
      "width": resizerWeight + "px",
      "top": (0.1 * height) + "px",
      "left": (width - resizerWeight) + "px"
    }
    , {
      "direction": "n-resize",
      "orientation": "horizontal",
      "height": resizerWeight + "px",
      "width": (0.8 * height) + "px",
      "top": "0px",
      "left": (0.1 * width) + "px"
    }
...
/**
 * load and display list of actions possible on a visual
 * @param visualIndex index of visual in list
 */
...
/**
 * load and display list of options/properties of a visual
 * @param visualIndex index of visual in list
 */
function showOptionsAt(visualIndex) {
  ...
  /**
   * collapse/expand to toggle options panel
   */
  options.select("div.options-title")
    .selectAll("div.close-button").data(["-"]).enter().append("div").classed("close-button", true)
  ...
  var newIcons = icons.enter().append("div").append("img")
    ...
    .on("mouseover", function (d) {
      d3.select(this).classed("focus2", true);
    })
  ...
  // display actions possible on a visual
...
function changeRender(visual, renderAs) {
  ...
  d3.selectAll("div.visual-container").filter(function (d) {
    return d.id == visual.id
  }).call(renderVisualData);
}
...
function setDropPosition() {
  ...
  if (field && options.datum() && options.datum().mouseover) {
    var placeholder = options.select("div.options-details").selectAll("div.options-detail-line").selectAll("div.value-placeholder")
      .filter(function (d) {
        return d.acceptingField && d.mouseover
      });

    if (placeholder.size() > 0) {
      //If we are here we are dragging over palce holder that accepts the current dragging field
      var vals = visuals[currentVisualOptionIndex].renderAs.fields.filter(function (v) {
        return v.name == placeholder.datum().name
      })[0].values;
...
function renderFieldFunctions(fieldData, holderData, valueItem) {
  ...
  if (currentChoice && options.filter(function (v) {
    return v.name == currentChoice
  }).length == 0) {
    options.filter(function (v) {
      return v.isOther
    })[0].name = currentChoice;
  }
  ...
  newChoices.append("input").attr("type", "radio").attr("name", function (d, i) {
    return d.randomName;
  });
  ...
  allChoices.selectAll("input")
    .property("checked", function (d) {
      return d.name == currentChoice
    })
    .style("display", fieldData.showDetails);
  allChoices.selectAll("span").text(function (d) {
    return d.name
  });
  choices.exit().remove();
}
